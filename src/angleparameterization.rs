...
impl AngleParameterization
{
    ...
    fn search_for_index(&self, target: f64) -> usize
    {
        ...
        while left < right {
            ...
            if self.total_angles[middle] == target { return middle; }
    ...
    pub fn get_angle_from_t(&self, t: f64) -> f64
    {
        ...
        let angle_end = if index != self.total_angles.len() - 1 { self.total_angles[index + 1] } else { 1. };
    ...
    pub fn find_parameters_for_angle_intervals(&self, angle_interval: f64) -> Vec<f64> {
        ...
        for (i, &total_angle) in self.total_angles.iter().enumerate() {
            if total_angle >= next_angle_threshold {
                ...
                let curve_parameter = (i as f64 - 1.0 + fraction) / (self.total_angles.len() as f64 - 1.0);
    ...
    pub fn find_parameters_for_angle_intervals_in_range(&self, angle_interval: f64, min_t: f64, max_t: f64) -> Vec<f64> {
