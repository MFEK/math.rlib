...
pub fn get_curve_control_point(knots: Contour<()>) -> Result<(Contour<()>, Contour<()>), String> {
    ...
    if n == 1 {
        ...
        // 3P1 = 2P0 + P3
        first_control_point[0].x = (2. * knots[0].x + knots[1].x) / 3.;
        first_control_point[0].y = (2. * knots[0].y + knots[1].y) / 3.;

        // P2 = 2P1 – P0
        second_control_point[0].x = 2. * first_control_point[0].x - knots[0].x;
        second_control_point[0].y = 2. * first_control_point[0].y - knots[0].y;
    } else {
    ...
    Ok((first_control_point, second_control_point))
}
