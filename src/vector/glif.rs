use super::Vector;
use glifparser::{
    glif::point::quad::QPoint, Handle, Point as GPPoint, PointData as GPPointData,
    PointType as GPPointType, WhichHandle,
};
...
impl Vector {
    ...
    pub fn from_quad_handle<PD: GPPointData>(point: &QPoint<PD>) -> Self {
        match point.a {
            Handle::At(x, y) => Self::from_components(x as f64, y as f64),
            Handle::Colocated => return Self::from_quad_point(point),
        }
    }

    pub fn to_point<PD: GPPointData>(
        self,
        handle_a: Handle,
        handle_b: Handle,
        ptype: GPPointType,
