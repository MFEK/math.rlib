...
impl GlyphBuilder {
    ...
    pub fn line_to(&mut self, to: Vector) {
        ...
        // self.beziers.push(line);//why not bazier_to
        self.bezier_to(line);
    }
    ...
    pub fn miter_to(&mut self, to: Vector, tangent1: Vector, tangent2: Vector) {
        ...
        if let Some(intersection) = _intersection {
            ...
            if from.distance(intersection) < from.distance(to) {
    ...
    pub fn arc_to(&mut self, to: Vector, tangent1: Vector, tangent2: Vector) {
        ...
        let tangent1_right = Vector {
            x: tangent1.y,
            y: -tangent1.x,
        }
        .normalize();
        let tangent2_right = Vector {
            x: tangent2.y,
            y: -tangent2.x,
        }
        .normalize();
        ...
        let circle_center = match intersection {
            Some(circle_center) => {
                ...
                if tangent1.distance(-tangent2) < SMALL_DISTANCE {
                    from.lerp(to, 0.5)
                } else if tangent1.distance(tangent2) < SMALL_DISTANCE {
                    from.lerp(to, 0.5)
                } else {
                    circle_center
                }
    ...
    pub fn circle_arc_to(&mut self, to: Vector, tangent1: Vector, tangent2: Vector) {
        ...
        for n in 0..repetitions as usize {
            ...
            if first_point {
                ...
                self.beziers
                    .push(Bezier::from_points(last.w1, last.w2, last.w3, cp1));
            }

            let cp2 = vec2!(f64::cos(next_angle), f64::sin(next_angle)) * radius + circle_center;

            let handle_distance = (4. / 3.) * f64::tan(consts::PI / 8.);
            let circle_tangent1 = -Vector {
                x: f64::sin(cur_angle),
                y: -f64::cos(cur_angle),
            }
            .normalize();
            let circle_tangent2 = Vector {
                x: f64::sin(next_angle),
                y: -f64::cos(next_angle),
            }
            .normalize();
        ...
        let difference = f64::atan2(
            f64::sin(last_angle - ending_angle),
            f64::cos(last_angle - ending_angle),
        );
        ...
        let circle_tangent1 = -Vector {
            x: f64::sin(last_angle),
            y: -f64::cos(last_angle),
        }
        .normalize();
        let circle_tangent2 = Vector {
            x: f64::sin(ending_angle),
            y: -f64::cos(ending_angle),
        }
        .normalize();
        let h1 = cp1 + circle_tangent1 * radius * handle_distance;
        let h2 = cp2 + circle_tangent2 * radius * handle_distance;
    ...
    fn find_discontinuity_intersection(
        from: Vector,
        to: Vector,
        tangent1: Vector,
        tangent2: Vector,
